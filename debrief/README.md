...
| `plans/` | The plans the deck and the videos were built from, after the persona reviews: `DECK_PLAN.md`, `DECK_ITERATIONS.md`, `LAUNCH_VIDEO_PLAN.md`, `WALKTHROUGH_PLAN.md` |

The diagrams used in the technical deck live with the architecture documents: `docs/architecture/diagrams/views/` (solution architecture, deployment, CI/CD), embedded in `docs/architecture/ARCHITECTURE.md` and `docs/operations/OPERATIONS.md`.

How this material was made, which tools and skills, and what was rejected: `AI_USAGE.md` section 1 (the second-session row) and section 4 ("Debrief material"). The working files (recordings, compositions, generators, drafts) stay in a git-ignored `notes/` folder on the author's machine; the plans above reference some of those paths.

Footage was recorded on the development environment with its own demonstration data; production was never touched.
