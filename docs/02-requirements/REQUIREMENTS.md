...
| FR-030 | An admin can open any application read-only (officer view, no actions) and manage users: list users, change a user's role, deactivate and reactivate a user. Accounts are created from the users page (at least 12 characters for the temporary password, audited as `user.created`; added 21 Sep 2026 at the owner's request) or by `scripts/create_user.py`. Every user change is an audit event. The last active admin cannot be demoted or deactivated; the seeded demonstration accounts are protected; an admin cannot change their own account. Operators and officers receive 403 on admin endpoints. | Not in the brief; product decision |
...
| SEC-006 | Passwords are hashed with argon2. JWT access tokens expire after 8 hours and are signed with `JWT_SECRET` from the environment; the application refuses to start without it in every environment; no fallback secret exists in the code. From v0.4.0 (US-093) a token carries its session id and is refused once that session is revoked (take-over, sign-out) or idle, whatever its expiry. |
...
| AUD-007 | Every checklist and clarification event (created, submitted, requested, released, answered, attachment added, resolved, reopened, withdrawn) and every site visit appointment event is an audit row in the same transaction; a draft save of the checklist is not audited (the draft is not a record until submitted). Session events (`user.session_taken_over`, `user.signed_out`, US-093) are audit rows with no application, carrying device labels only. (v0.4.0) |
