...
| File | What it is |
|------|------------|
| `pitch-deck/PermitFlow-pitch.pptx`, `PermitFlow-pitch-handout.pdf` | Eleven slides for the debrief audience: the problem, the product, the AI check, a real workflow, principles, architecture, quality, next steps. Speaker notes in the pptx. |
| `technical-deck/PermitFlow-technical.pptx`, `PermitFlow-technical-handout.pdf` | Eighteen slides on how it is built: the brief decoded, scope, assumptions, how the work was run (the board), architecture, four ADRs, branching, deployment, CI, readiness evidence, AI usage (building the product; the debrief material), next steps. Speaker notes in the pptx. |
