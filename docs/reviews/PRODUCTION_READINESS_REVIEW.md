...
| 9 | AI quality | 14-case golden set; confidence uncalibrated; English-only injection heuristic; images unreadable | Medium | Blocking in CI on the mock; live run recorded per prompt version | Labelled set from officer overrides, promptfoo gate and red-team suite, LangSmith or OpenTelemetry traces, calibration, OCR |
...
| 16 | Supply chain | pip-audit and npm audit non-blocking; no SAST | Low | gitleaks blocking; pinned dependencies (`uv.lock`, `package-lock.json`) | Blocking audits once the baseline is clean, Semgrep or CodeQL, image scanning, SBOM |
