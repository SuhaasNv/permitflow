...
By the numbers: 730 backend tests on a real PostgreSQL, 51 frontend tests, 7 Playwright specs run in CI against the full stack, 14 AI golden cases (12 counted on the mock, 14 of 14 live), 20 threats with controls, 11 ADRs, 63 stories on a Notion board mirrored in the repository, 150+ conventional commits on story branches merged `--no-ff`.
