...
Verification: `backend/tests/integration/test_edge_cases.py`, `frontend/src/lib/unsaved.test.ts`, `frontend/src/features/operator/queries.test.ts`, plus the existing suites. Fixed on `fix/operator-edge-cases` and `fix/backend-edge-cases`.
