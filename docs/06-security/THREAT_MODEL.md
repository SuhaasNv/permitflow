...
- **Gap:** A session that ends by take-over or expiry keeps the copies on purpose (the same person signs in again to carry on); a production build on managed devices would encrypt them with a per-session key.
