...
| FR-030 | An admin can open any application read-only (officer view, no actions) and view a read-only user directory. Operators and officers receive 403 on admin endpoints. | Not in the brief; product decision |
...
| SEC-005 | Uploads are restricted by MIME type and extension allowlist (PDF, PNG, JPEG, TXT), size limit (10 MB), and are stored with server-generated names outside the web root. Files are served only through an authorized endpoint. |
