...
**Prototype v0.4.0 (20 Sep 2026, US-078; the What's new row added 21 Sep, US-094):** https://claude.ai/artifact/VmZ2biUWNkGMTAFoa6D8LY: nineteen artboards for the v0.4.0 screens (the officer's checklist at 820 portrait and 1024 landscape, the clarification rail, the operator's respond screen at 390 and 1280 with the send dialog, the operator's history, the four admin screens), generated from `docs/04-design/prototype-src/v0-4-0/build.py` with the as-built tokens, so a change is made in the generator and republished, never drawn by hand. The states matrix for these screens is in `UI_STATES.md`; the flows in `UI_FLOW.md`; the components in `COMPONENT_INVENTORY.md` (v0.4.0 section).
