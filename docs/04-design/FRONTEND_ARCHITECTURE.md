...
React 19, TypeScript strict (no `any`), Vite, Tailwind v4, TanStack Query, React Hook Form with Zod, React Router. Tests: vitest with Testing Library (152 tests in 32 files, coverage thresholds in `vite.config.ts`), Playwright (the journey, six scenarios, an axe accessibility gate over 23 screen states). Linting: oxlint.
