...
One record of what a person actually does and sees, step by step, in the product as it runs today. Written after Sprint 2, from the browser. Screen IDs refer to `SCREEN_INVENTORY.md`; statuses use the role-specific labels from `../03-architecture/STATE_MACHINE.md`. Two personas only: the operator (the person who fills in and submits the application, SCOPE assumption 10) and the licensing officer. The admin persona is Sprint 3.
...
| 17 | Application S-11 | Downloads the licence certificate (PDF) from the outcome panel once approved | Issued by the approval transaction, one year validity, verification code on the document (US-051) | Approved |
...
| 10 | Feedback rail | Mark resolved on addressed items, or Not fixed to reopen one with the same text for the next round (US-049); a warning stays while addressed items are unresolved | `feedback.resolved` audited; open items block the site visit, addressed ones only warn | Under Review |
| 11 | Review rail | Mark site visit scheduled → Mark site visit done → Route to approval → Preview licence (watermarked) → Approve (note optional, issues the licence; the dialog warns when check results are unresolved) or Reject (note required, from any non-terminal state) or Return to review (back to Under Review for another feedback round) | Each is a state-machine edge with a guard; the operator sees Pending Site Visit, Pending Post-Site Clarification is unreachable in v0.3.0 (UC3 deferred), then Pending Approval, then the outcome | Site Visit Scheduled → Site Visit Done → Route to Approval → Approved / Rejected |
| 11 (v0.4.0) | Checklist S-30, rail S-31, respond S-18 | The officer fills the checklist on site and submits it; the operator answers only the flagged items with evidence and sends; the officer marks items clarified or asks again over as many rounds as needed, then routes to approval | Designed 20 Sep 2026 (US-078), built in Sprints 5 and 6; the flows are in `UI_FLOW.md`; the direct route from Site Visit Done to approval goes with US-079 | Site Visit Scheduled → Site Visit Done → Awaiting Post-Site Clarification → Post-Site Clarification Resubmitted (→ Awaiting Post-Site Resubmission …) → Route to Approval |
...
- No site-visit checklist or per-item clarification rounds in v0.3.0 (UC3, deferred): the visit is a status change, not a form. Designed for v0.4.0 on 20 Sep 2026 (step 11 above).
...
- The admin persona (oversight, user management) was cut at the Sprint 3 close; designed for v0.4.0 on 20 Sep 2026 (`UI_FLOW.md`, Admin flow).
