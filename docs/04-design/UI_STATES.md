...
| S-33 (v0.4.0) | card skeleton | no visit yet (card absent) | counter-proposal too soon ("at least two working days ahead"), reason missing, 409 when the officer decided first (reload banner) | Waiting for your reply (accept or counter), Waiting for the officer (your proposal shown), Confirmed (date and slot, Request a different date), Done |
| S-34 (v0.4.0) | panel skeleton | none | 409 when the operator replied first; Confirm without a reply disabled with "available from …" | Waiting for you (three choices), Waiting for the operator, Confirmed; Mark site visit done enabled only once confirmed |
