...
5. A release is the pull request `dev` into `main`, merged when green, then a git tag `v0.<release>.0` on that merge commit (the first three releases were one per sprint, `v0.1.0` to `v0.3.0`; from v0.4.0 a release spans several sprints, and `dev` carries the working version `0.4.0-dev` until the bump; a release candidate may be tagged on `dev` as `v0.4.0-rc.N` with the version bumped to match, which builds `:v0.4.0-rc.N` images for the development environment and touches neither `main` nor production, as `v0.4.0-rc.1` was on 21 Sep 2026). Before the release: bump `frontend/package.json` `version` to the same number and add the version's entry to `RELEASE_NOTES.md` (what is new, improved, and known limitations, in the users' words); a release without its notes is not tagged. The version: it is shown in the app shell and landing footer, and CI refuses a tag that does not match it. Only a push of a `v*` tag makes CI write the `:v0.x.0` image tags (branch pushes write `sha-<commit>` and the branch name), so a release image can never be overwritten by a later merge. Production runs a pinned `sha-<commit>` image; a release changes that pin, then the approved deploy job rolls it out; rollback is the previous pin and the same job. A migration in a release must keep the schema readable by the previous release (`OPERATIONS.md`, Migrations), so the rollback stays safe.
