...
|---------------|----------------------------|---------------|----------------|
| `draft` | — (assumption) | — (not listed for officers) | Draft |
...
| `site_visit_scheduled` | `site_visit_done` | officer | — | Officer clicks Mark site visit done |
| `site_visit_scheduled` | `rejected` | officer | — (note required) | Officer clicks Reject |
| `site_visit_done` | `rejected` | officer | — (note required) | Officer clicks Reject |
...
| `site_visit_done` | `pending_approval` | officer | — | Officer clicks Route to approval (MVP assumption: no checklist) |
| `awaiting_post_site_clarification` | `pending_post_site_resubmission` | officer | — | UC3 (deferred) |
| `awaiting_post_site_clarification` | `pending_approval` | officer | — | UC3 (deferred) |
...
- An officer may resolve only items that were released to the operator (an unsent draft can only be withdrawn), and may undo their own withdraw or resolve within 15 seconds while the state still allows the original action; the undo is audited as `feedback.restored` (US-039).
