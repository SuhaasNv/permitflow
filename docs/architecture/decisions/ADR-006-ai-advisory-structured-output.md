...
- Evaluation set in `backend/evals/` with fourteen cases and a runner script (`python -m evals.run`), blocking in CI on the mock provider and run by hand against OpenAI on every prompt change; results recorded in `docs/ai/AI_EVALUATION.md`.
