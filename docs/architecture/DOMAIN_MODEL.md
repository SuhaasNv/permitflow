...
Ownership: a user owns their notifications. Operators own the applications they create.
...
| sha256 | str | integrity and duplicate detection |
...
| event_type | str | `application.created`, `revision.submitted`, `status.changed`, `feedback.created`, `feedback.released`, `feedback.addressed`, `feedback.resolved`, `feedback.withdrawn`, `document.uploaded`, `document.replaced`, `document.deleted`, `verification.completed`, `decision.recorded` |
...
| User | self | self | all; read-only directory |
