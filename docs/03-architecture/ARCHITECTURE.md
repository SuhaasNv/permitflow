...
| POST | /applications/{id}/clarifications/responses/{rid}/attachments | operator (own) | multipart `file`: the document rules (allowlist, magic bytes, 10 MB), three per answer (422 `attachment_cap`), an identical file is kept once and reported `unchanged`; 409 once sent; audit `clarification.attachment_added` (built, US-065) |
