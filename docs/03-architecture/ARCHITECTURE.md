...
| POST | /officer/applications/{id}/clarifications/{item_id}/restore | officer | undo of a withdraw inside the 15 s window (the page offers Undo for 10 s): the question is open again, released or still a draft as it was; 409 once the window closed or the round moved on; audit `clarification.restored` (UAT run 5, F19) |
...
| GET | /admin/audit-feed?limit=50&before=<cursor> | admin | the newest audit events across every application and every user change, with the sentence, actor, case reference; keyset cursor `<micros>,<id>` (never `OFFSET`), `next_cursor` null at the end; 400 `bad_cursor`; a draft's events stay out until it is submitted, like every other admin read of a draft (built, US-072; security audit, 24 Sep) |
...
A case can have more than one site visit (Return to review after Route to approval, then a new appointment). The views read one visit, the *active* one (`services/visit_scope.py`): none while the case is back in the pre-visit review (Application Received to Pre-Site Resubmitted), otherwise the latest visit. `site_visit`, `checklist` and `clarification` on both application views are the active visit's; `earlier_visits` lists the others, latest first and read-only: the operator view carries each visit's appointment and clarification thread (`EarlierVisitOperatorView`), the officer view also the checklist summary, readable in full at `GET /officer/applications/{id}/checklist?visit=N` (`EarlierVisitOfficerOut`). Guards keep their own reading of the current visit. The appointment views also carry `date_stands` (confirmed, or a move request pending on a confirmed date) and, for the officer, `visit_day_reached`.
