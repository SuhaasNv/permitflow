...
| POST | /officer/applications/{id}/site-visit/decide | officer | `{ action: accept_operator \| keep_original \| propose, date?, slot?, note? }` on a counter-proposal: confirms the operator's date, keeps the date on the table, or proposes a third one (round cap: six proposals per visit, 409 `Round limit reached` beyond it); audit `site_visit.confirmed` (`how`) or `site_visit.proposed`; operator notified (built, US-084) |
