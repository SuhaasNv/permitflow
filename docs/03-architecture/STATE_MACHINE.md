...
| `site_visit_scheduled` | `site_visit_done` | officer | `visit_confirmed` (the confirmed date stands: the current SiteVisit is `confirmed`, or the operator asked to move a confirmed visit and the officer has not decided yet, UC3-0 4a; US-084) and, while `SITE_VISIT_DAY_GUARD` is on (the default), the visit day has arrived (Singapore date; refused with "The visit is on Wed 30 Sep. Mark it done on or after that day.", UAT run 5, F12) | Officer clicks Mark site visit done (or submits the checklist); the visit row becomes `done` in the same transaction, and a move request still waiting is closed as declined (`site_visit.move_request_closed`) |
