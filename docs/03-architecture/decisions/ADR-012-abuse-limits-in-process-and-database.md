...
`tests/unit/test_rate_limit.py` (window arithmetic, bucket choice, forwarded-for hop, map bound), `tests/integration/test_abuse_limits.py` (sign-in flood, general limiter, draft cap, daily quota, headers), `docs/06-security/SECURITY_REVIEW.md` (the four attacks before and after).
