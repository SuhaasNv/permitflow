...
| `permitflow_build_info` | gauge, always 1 | `version`, `commit` | which build answers: the version from `app/core/version.py` and the commit CI baked into the image (`GIT_SHA`; `local` outside an image), so the bot and the digest can name it (owner's request, 21 Sep 2026) |
...
- **An hourly digest from Grafana.** Grafana OSS has no scheduled reports, so the digest is an always-firing rule per environment (`PermitFlow hourly digest (development)`, `(production)`, `(local)`) whose annotations carry the numbers, and a notification policy that repeats it every hour. One message with a section per environment that Prometheus is scraping (every number guarded: an evaluation during which a query has no result, as when Prometheus restarts, prints "n/a" instead of the raw template, which one digest did on 21 Sep 2026): API up or down, the version and commit answering (`permitflow_build_info`, as the labels of a query the annotation reads), requests per minute, p99.5 latency, checks and their spend in the hour, applications, who is waiting. An environment that is not scraped stays silent (NoData is OK). Change `repeat_interval` on the digest route to `24h` for a morning summary.
- **Commands, answered by a small bot.** `docker/observability/telegram-bot/bot.py`, standard library only, long-polls the Bot API and answers only the configured chat: `/status` (every environment, each with the version and commit its API reports), `/dev`, `/prod`, `/checks` (outcomes and p95 in the last hour), `/cost` (tokens and spend in the last 24 h, per check, projected a month), `/queue` (applications by status, transitions in the last hour, the post-site cases from Site Visit Scheduled to Post-Site Clarification Resubmitted, checklists and rounds in the last 24 hours), `/alerts` (what Prometheus has firing), `/help`. It reads Prometheus over the private network and writes nothing anywhere. The command menu is registered with the Bot API (`setMyCommands`), so Telegram shows the list.
