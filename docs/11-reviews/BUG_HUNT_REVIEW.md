...
A full operator and officer journey in Chrome with `docs/12-demo/documents` (clean set, then the wrong file in the floor plan slot, Not fixed, a second resubmission, site visit, preview, approval, download) after the bug-hunt batch. Two more defects, fixed on `feat/us-051-licence-certificate`:
...
Run 3 on the deployed development environment (26 steps, two personas, `docs/10-uat/UAT_PLAN.md`) added one more:
