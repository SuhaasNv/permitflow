...
| H4 | /officer/queue | 1280, rail expanded (found 20 Sep 2026 by the owner on the development environment while reviewing the v0.4.0 prototype) | The next-action chip ("Continue review", "Review resubmission") sits under the reference instead of in the row and, when wider than the reference column, overflows to the left and is clipped ("eview resubmission") | Give the chip its own column from `lg`, or let it wrap inside the reference cell with `min-w-0` and left alignment; regression test at 1280 | fixed 21 Sep 2026 (found again by the owner at 1024 with "Post-Site Clarification Resubmitted" running under the button): the status column wraps its badge and the action column has a fixed width at every breakpoint, so header and rows stay aligned |
