...
| Final-check findings kept as they are: queue reads the working copy, draft quota counted without a lock, undo of "Not fixed" resets the release time, `Retry-After` not surfaced, a few services issue SQL directly | Each is cosmetic, self-healing or bounded; listed with the production fix in `PRODUCTION_READINESS_REVIEW.md` row 23 |
