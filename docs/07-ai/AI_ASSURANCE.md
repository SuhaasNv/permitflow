...
One page for the question "how do you evaluate the AI, and what stops a bad answer from mattering?". Written 19 Sep 2026 (US-054, US-055, US-056). Detail lives in `AI_VERIFICATION_DESIGN.md` (the pipeline), `AI_EVALUATION.md` (the numbers), `../06-security/THREAT_MODEL.md` (T16, T18, T21) and `../03-architecture/decisions/ADR-006`.
...
| 3. Fairness check | The same document with the applicant's and the business's names swapped for names from Singapore's Chinese, Malay, Indian, Eurasian and Western communities must get the same status and codes: one baseline plus 6 swapped name sets across 3 scenarios, 21 rows of which 18 are swaps, every swap must match its scenario's baseline | `evals/fairness.py` |
...
|---|---|---|
| promptfoo | Own harness (`evals/run.py`, `evals/fairness.py`) | Same job (cases, expected outcomes, blocking gate) through the real pipeline in about 400 lines of typed Python; no second config language, no second runtime |
...
| Risk score with weights | Not done | Pass or fail per stage is the right grain at this scale; a weighted score over 14 cases would be precision the data does not have |
| Project Moonshot, IMDA Starter Kit | Named as the next step | The Singapore assurance framing for a real deployment (`../11-reviews/PRODUCTION_READINESS_REVIEW.md`, gap 9) |
