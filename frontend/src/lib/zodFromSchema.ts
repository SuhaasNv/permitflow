...
function fieldSchema(f: FieldDef, mode: FormMode): z.ZodTypeAny {
  ...
  switch (f.kind) {
    case "checkbox": {
      ...
      if (f.must_be_true && mode === "complete")
        return base.refine(
          (v) => v === true,
    ...
    case "integer": {
      ...
      if (f.min_value !== null)
        num = num.min(f.min_value, `Must be at least ${f.min_value}.`);
      if (f.max_value !== null)
        num = num.max(f.max_value, `Must be at most ${f.max_value}.`);
    ...
    default: {
      ...
      if (f.max_length !== null)
        str = str.max(
          f.max_length,
          `Must be ${f.max_length} characters or fewer.`,
        );
      const withRules = str.refine(
        ...
        f.kind === "email"
          ...
          : (f.pattern_message ?? "Invalid format."),
      );
      return optional
        ? withRules.optional()
        : withRules.refine((v) => v !== "", REQUIRED);
    }
  }
}
...
export function defaultsFor(
  ...
  for (const f of section.fields) {
    ...
    if (f.kind === "checkbox") out[f.key] = v === true;
      ...
      out[f.key] = typeof v === "number" ? v : undefined;
