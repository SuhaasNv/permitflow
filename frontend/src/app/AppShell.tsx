...
const NAV: Record<Role, NavItem[]> = {
  operator: [
    { label: 'Dashboard', to: '/app/dashboard', icon: icon('m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2zM9 22V12h6v10') },
    { label: 'My applications', to: '/app/applications', icon: icon('M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z') },
  ],
  officer: [
    { label: 'Review queue', to: '/officer/queue', icon: icon('M22 12h-6l-2 3h-4l-2-3H2M5.5 5.1 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.5-6.9A2 2 0 0 0 16.8 4H7.2a2 2 0 0 0-1.7 1.1z') },
  ],
  admin: [
    { label: 'Overview', to: '/admin/overview', icon: icon('M22 12h-4l-3 9L9 3l-3 9H2') },
  ],
}

const ROLE_LABEL: Record<Role, string> = { operator: 'Operator', officer: 'Licensing officer', admin: 'Administration' }
...
export function AppShell() {
  ...
  const nav = (
    <nav
      aria-label="Main"
      className={cn(
        'flex flex-col gap-0.5 border-r border-line bg-surface p-3',
        collapsed ? 'w-16 items-center px-2' : 'w-[232px]',
      )}
    >
      {!collapsed ? (
      ...
      {!collapsed ? (
        <div className="mt-auto border-t border-line p-3 text-xs leading-[18px] text-text-3">© 2026 PermitFlow (fictional prototype)</div>
      ) : null}
    </nav>
  )
  ...
  return (
    <div className="flex min-h-screen flex-col bg-bg">
      <div className="flex h-7 items-center gap-2 bg-text px-4 text-xs text-[#c5cbd3] sm:px-6">
        ...
        <span className="ml-auto hidden sm:inline">Session expires after 8 hours</span>
      </div>
      <header className="flex h-14 items-center gap-3 border-b border-line bg-surface px-3 sm:gap-4 sm:px-6">
        ...
        <div className="ml-auto flex items-center gap-2">
          <div className="flex items-center gap-2.5 rounded-md px-1 py-1">
            <span className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e4e7ec] text-xs font-semibold text-text-2">{initials(user.full_name)}</span>
