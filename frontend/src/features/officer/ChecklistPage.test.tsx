...
describe('site visit checklist (US-060)', () => {
  ...
  it('opens the draft with a POST, renders every item, and saves the whole list with the version and a save id', async () => {
    ...
    await userEvent.click(within(groups[0]!).getByRole('button', { name: 'Satisfactory', exact: true }))
    await userEvent.click(within(groups[1]!).getByRole('button', { name: 'Unsatisfactory', exact: true }))
    expect(within(groups[1]!).getByRole('button', { name: 'Unsatisfactory', exact: true })).toHaveAttribute('aria-pressed', 'true')
  ...
  it('a stale version shows the other version with Keep my entries or Take theirs', async () => {
    ...
    expect(within(groups[0]!).getByRole('button', { name: 'Satisfactory', exact: true })).toHaveAttribute('aria-pressed', 'true')
    await userEvent.click(screen.getByRole('button', { name: 'Save draft' }))
    await waitFor(() => expect(save).toHaveBeenCalledTimes(2))
