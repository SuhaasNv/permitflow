...
export function ChecklistCard({ view }: { view: OfficerApplication }) {
  ...
  const to = `/officer/applications/${view.id}/checklist`
  return (
      ...
      <div className="flex flex-col gap-3 px-5 pb-5 pt-2">
        {c ? (
          <>
            ...
            <Link to={to} className={buttonClasses(c.status === 'submitted' ? 'secondary' : 'primary')}>
              {c.status === 'submitted' ? 'View checklist' : 'Continue checklist'}
            </Link>
          </>
        ) : (
          <>
