...
export function LoginPage() {
  ...
  const onSubmit = form.handleSubmit(async (values) => {
    ...
    try {
      ...
      navigate(from && from !== '/login' ? from : homeFor(response.user.role), { replace: true })
    } catch (error) {
  ...
  return (
    <div className="flex min-h-screen bg-bg">
      <section className="flex w-full flex-col border-r border-line bg-surface px-6 py-10 sm:px-12 lg:w-[520px] lg:px-16 lg:py-14">
        ...
        <div className="mt-auto flex flex-col gap-1.5 pt-10 text-xs leading-[18px] text-text-3">
