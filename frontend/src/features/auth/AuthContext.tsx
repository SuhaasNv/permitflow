...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = useCallback((response: TokenResponse) => {
    const next = { token: response.access_token, user: response.user, expiresAt: response.expires_at }
    setSession(next)
    writeStored(next)
  }, [])

  const value = useMemo<AuthState>(
    () => ({ user: session?.user ?? null, token: session?.token ?? null, ready, signIn, signOut }),
