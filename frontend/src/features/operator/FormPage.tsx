...
function Stepper({ current }: { current: number }) {
  return (
    <ol
      className="flex flex-wrap items-center gap-y-2 text-[13px]"
          ...
          <li key={label} className="flex items-center">
            ...
            {i < STEPS.length - 1 ? (
              <span
                className={cn(
                  "mx-2 h-px w-7",
                  state === "done" ? "bg-success" : "bg-line-strong",
                )}
                aria-hidden="true"
              />
            ) : null}
...
export function FormPage() {
  ...
  if (schema.isError)
    return (
      <ErrorPanel error={schema.error} onRetry={() => void schema.refetch()} />
    );
  ...
  const save = async (
    ...
    if (andContinue) {
      ...
      navigate(
        next
          ? `/app/applications/${id}/form/${next.key}`
          : `/app/applications/${id}`,
      );
    }
  };
  ...
  return (
    <>
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-text-2">
            {view.sections[0]?.data.business_name
              ? String(view.sections[0].data.business_name)
              : "New application"}
          </p>
        </div>
        <Link
          ...
          >
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    active &&
                      "bg-neutral-soft font-semibold text-text shadow-[inset_3px_0_0_var(--color-primary)]",
                  )}
                >
                  <span>{s.title}</span>
                  <span
            ...
            >
              ...
              <span className="ml-auto text-xs text-text-3">
                {view.completeness.documents_present}/
                {view.completeness.documents_total}
              </span>
            </Link>
          </nav>
      ...
      >
        <p>
          Changes you made in this section have not been saved. If you leave now
          they will be lost.
        </p>
      </Dialog>
    </>
