...
const STEPS = [
  [
    "Apply",
    "Four short sections: business, premises, operations and declarations. Each one validates as you go.",
  ],
  [
  ...
  return (
    <div className="flex min-h-screen flex-col bg-surface text-text">
      ...
      <header className="border-b border-line">
        <div className="mx-auto flex h-16 max-w-[1080px] items-center justify-between px-4 sm:px-8">
          ...
          >
            <a
              href="#how"
              className="hidden text-text-2 no-underline hover:text-text sm:inline"
            >
              How it works
            </a>
            <a
      ...
      <section className="border-b border-line">
        <div className="mx-auto grid max-w-[1080px] gap-10 px-4 py-14 sm:px-8 lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-16 lg:py-[72px]">
          <div>
            <div className="mb-3.5 text-xs font-semibold uppercase tracking-[0.08em] text-primary">
            ...
            <h1 className="max-w-[720px] text-[32px] font-semibold leading-10 tracking-tight sm:text-[40px] sm:leading-[48px]">
              Apply for a food establishment licence and respond to the
              licensing officer in one place.
            </h1>
            <p className="mt-4 max-w-[640px] text-base leading-7 text-text-2 sm:text-lg">
              Complete the application in sections, upload your documents and
              see automatic checks before you submit. If the licensing office
              needs changes, you update only what was flagged. Nothing you
              entered is lost.
            </p>
            ...
            <p className="mt-4 text-xs text-text-3">
              Takes about 20 minutes. You can save a draft and return later.
            </p>
          </div>
          <div
            id="need"
            className="rounded-lg border border-line bg-surface p-5 shadow-[var(--shadow-1)] lg:mt-11"
          >
            ...
            <p className="mt-3.5 text-xs leading-[18px] text-text-3">
              PDF, PNG, JPG or TXT, up to 10 MB each. PDF is recommended: it is
              the only format the automatic check can read.
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="how" className="border-b border-line">
        <div className="mx-auto max-w-[1080px] px-4 py-14 sm:px-8">
          <h2 className="mb-6 text-2xl font-semibold leading-8">
            How it works
          </h2>
          <ol className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {STEPS.map(([title, desc], i) => (
              <li key={title}>
                <div className="mb-2.5 font-mono text-[13px] font-medium text-primary">
                  0{i + 1}
                </div>
          ...
          <div className="grid max-w-[960px] gap-8 sm:grid-cols-3">
            <div>
              ...
              <p className="text-[13px] text-text-2">
                The licensing office is working on it. Nothing is needed from
                you.
              </p>
            </div>
            <div>
              ...
              <p className="text-[13px] text-text-2">
                Feedback is waiting for you. Only the flagged parts reopen for
                editing.
              </p>
            </div>
            <div>
              ...
              <p className="text-[13px] text-text-2">
                The outcome, with the officer's note, stays on your dashboard
                with the full history.
              </p>
            </div>
          </div>
          <p className="mt-5 max-w-[640px] text-text-2">
            Every application keeps a complete record: each revision you
            submitted, every comment from the licensing office, and when each
            status changed.
          </p>
        </div>
      </section>
      ...
      <footer className="mt-auto border-t border-line">
        <div className="mx-auto flex max-w-[1080px] flex-wrap items-center gap-x-6 gap-y-2 px-4 py-8 text-[13px] text-text-3 sm:px-8">
