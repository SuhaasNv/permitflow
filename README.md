...
See `.env.example`; every runtime variable is documented there and in `docs/operations/OPERATIONS.md` (`SEED_PASSWORD` is read by the seed script only). `JWT_SECRET` is required outside `APP_ENV=test`. The frontend needs no `.env` locally: it defaults to `http://localhost:8000/api/v1`.
...
**CI for the AI.** The six-stage AI gate on every push and the nightly live evaluation are described in the CI table above; the one-page account of every layer, what it proves and what it does not, is `docs/ai/AI_ASSURANCE.md`.
