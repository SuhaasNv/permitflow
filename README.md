...
794 backend cases from 204 test functions on a real PostgreSQL (the state-machine sweep alone is 597), 191 frontend tests, ten Playwright specs (the journey, eight scenarios, the accessibility gate), 204 API-level edge checks (`backend/scripts/uat_edges.py`). Coverage: backend 95 %, frontend 81 % statements, both enforced in CI. Layers, commands and what each protects: `docs/08-testing/TEST_STRATEGY.md`; manual acceptance record: `docs/10-uat/UAT_PLAN.md`.
