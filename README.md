...
**Try it:** development environment https://dev.permitflow.space (platform host as a fallback: https://frontend-development-afe2.up.railway.app), demo accounts below. Production goes live with the v0.3.0 release on https://permitflow.space. Local setup takes about ten minutes (below).
...
747 backend tests (646 unit, 101 integration on a real Postgres), 152 frontend tests, eight Playwright specs (the journey, six scenarios, the accessibility gate). Coverage on 19 Sep 2026: backend 96 % statements, frontend 80.6 % statements and 83.9 % lines, both measured over every source file and enforced in CI (`docs/testing/TEST_STRATEGY.md`, US-053).
...
**CI for the AI.** The AI gate (`ai-gate.yml`, called from `ci.yml` on every push) has six named stages on the mock provider: model approval (pinned wire schema, prompt version, default model, provider wiring), contract tests, the golden set (fourteen cases: the demo PDFs, edge cases and two prompt-injection styles) through the real pipeline, blocking below 100 % of the twelve counted cases (two are documented mock gaps, reported but not counted), the adversarial cases, the 21-run name-swap fairness check, and a verdict table. The live evaluation (`ai-eval.yml`) runs the same set and the fairness check against OpenAI nightly, by hand, and on any change to the AI module, blocking at 14 of 14 and 21 of 21, with the model and prompt version stamped in the result and each run recorded as a LangSmith experiment (`docs/ai/AI_EVALUATION.md`, `docs/ai/AI_ASSURANCE.md`). The dependency and code audit job (pip-audit, bandit, npm audit) blocks the build.
