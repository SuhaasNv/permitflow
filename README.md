...
752 backend test cases from 163 test functions (one function, the state-machine sweep, contributes 588 parametrised cases; 102 integration cases run on a real Postgres), 152 frontend tests, eight Playwright specs (the journey, six scenarios, the accessibility gate). Coverage on 19 Sep 2026: backend 96 % statements, frontend 80.6 % statements and 83.9 % lines, both measured over every source file and enforced in CI (`docs/08-testing/TEST_STRATEGY.md`, US-053).
