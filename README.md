...
5. **Evaluation and observability for the AI.** The golden set is a 14-case smoke test; next is a labelled set grown from officer overrides, promptfoo for a threshold gate and prompt red-teaming, LangSmith (or OpenTelemetry) traces per run, per-run token and cost accounting, confidence calibration, and image OCR for scanned uploads. A CJK-capable font for the certificate.
...
7. **Delivery.** A staging copy of production data for migration rehearsal, blue/green or at least two replicas, SAST (Semgrep or CodeQL) in CI, dependency audit made blocking once the baseline is clean, backup and retention policy for the database and files.
