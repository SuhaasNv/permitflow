...
- US-052 Custom domain (19 Sep): one convention per environment on the owner's domain, `permitflow.space` / `api.permitflow.space` (production) and `dev.permitflow.space` / `api.dev.permitflow.space` (development); five custom domains registered on Railway, six DNS records at the registrar (ALIAS at the apex, TXT verification, four CNAMEs), `CORS_ORIGINS` and `API_URL` set per environment with the railway.app hosts kept as fallbacks; GitHub environment variables and the deploy gates moved to the new hosts once the five certificates were issued; development is live on the domain (deploy #6 green, scenario 02 through it).
